...
class SyntheticDataGenerator:
  ...
  def generate(self, model_dir: str, output_dir: str):
    for _ in range(self.n_patients):
      ...
      for i, event_temperature in enumerate(self.temperatures, start=1):
        ...
        output_file = self.os.path.join(output_dir, userId + "_" + userno + ".csv")
        
        print(f'Generating File {userno} for user id {userId}' )
        try:
          data_generator = DataGenModel(max_timings, model_dir, event_temperature = event_temperature)
